...
export default function AgentDeliveredOrders(props) {
...
const downloadReport2 = () => {
  ...
  ApiClient.PostApi(ApiUrl.AgentOrdersCSV, agent).then((value) => {
    if (value !== null) {
      if (value.status === "success") {
        ...
        console.log("value.new_orders",value.delivered_orders)
        setCsvData(value.delivered_orders);
  ...
  return (
    ...
    <div className="card-box"> <div className="container p-0 delivery_container">
        <div className="row search-box mb-3">
          <h4 className="protitle mb-0">Delivered Orders</h4>
        </div>
        <div className="text-right">
              <CSVLink
                headers={headers}
                data={csvData}
                filename={"AgentDeliveredOrdersCargillDelivery.csv"}
                ref={csvLinkEl}
              >
                <button
                  type="button"
