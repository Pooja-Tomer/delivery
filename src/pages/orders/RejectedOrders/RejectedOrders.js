...
export default function RejectedOrders(props) {
  ...
  // const [currentState, setCurrentState] = useState(CATERGORY_NAME.REJECTED_ORDERS);
  ...
  useEffect(() => {
    ...
    ApiClient.PostApi(ApiUrl.OrdersList, params).then((value) => {
      if (value.status === "success") {
        ...
        var RejectedOrderData = [];

        const rejectedorders = value.rejected_orders;
        if (rejectedorders) {
          if (rejectedorders.length > 0) {
            RejectedOrderData = rejectedorders;
          }
        }

        const CategData = [
         
          { Name: CATERGORY_NAME.REJECTED_ORDERS, Data: RejectedOrderData },
        ];
    ...
    ApiClient.PostApi(ApiUrl.OrdersCSV, partner).then((value) => {
      if (value !== null) {
        if (value.status === "success") {
          ...
          console.log("value.new_orders",value.rejected_orders)
          setCsvData(value.rejected_orders);
  ...
  return (
    ...
    <div className="card-box"> <div className="container p-0">
        <div className="row search-box ">
          <h4 className="protitle mb-0">Rejected Orders</h4>
          <div className="text-right">
              <CSVLink
                headers={headers}
                data={csvData}
                filename={"RejectedOrdersCargillDelivery.csv"}
                ref={csvLinkEl}
              >
                <button
                  type="button"
